...
* [예제 4-25 오류 상황 테스트]
*
* 테스트 코드에서 필요한 공통 로직을 메소드로 추출했다.
* 그리고 좋은 진단을 위해 JUnit assertThat()을 사용했다.
* assertThat()은 첫 번째 인수에 값을 받고 두 번째 인수로 Matcher를 받는다.
* hasSize() 매처는 Matchers 유틸리티 클래스에서 정적 임포트한 메소드로 컬렉션의 크기가 파라미터 값과 일치하는지 확인한다.
* 또한, 오류 상황을 체크하기 위해 JUnit @Test 어노테이션의 expected = 속성을 사용했다.
* */
